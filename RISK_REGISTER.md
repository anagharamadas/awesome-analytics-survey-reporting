...
| D-13 | **3,236 responses (8.6%) predate their own survey's `created_date`.** Survey 12 was created 01/04/2026 and has 1,335 responses before that; survey 10 has 916. Every survey's first reporting week is 2025-12-27. So either `created_date` is not a launch date, or the timestamps are back-dated. | High | Med | Loaded, not rejected — discarding 8.6% of the dataset on an inferred rule would be far worse than surfacing it, and `created_date` is the less trustworthy column after three migrations. Consequence: `created_date` is **not** used as a report boundary anywhere. Raised for the client in README section 4. | Open |
